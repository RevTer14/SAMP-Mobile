...
void CStreaming::RemoveBigBuildings() {
    // CStreaming::RemoveBigBuildings(void)	00000000003978F0	X64
    // CStreaming::RemoveBigBuildings(void)	002D52A0	X32
    ...
    CHook::CallFunction<void>(g_libGTASA + (VER_x32 ? 0x002D52A0+1:0x3978F0));
}
...
void CStreaming::Update() {
    FLog("CStreaming::Update");
    if (CTimer::GetIsPaused())
        return;

    FLog("CStreaming::Update1");
...
//    }
    ...
    const CVector& playerPos = ((ENTITY_TYPE *)pLocalPed->m_pPed)->m_placement.m_vPosn;
...
//    }
    ...
    if (pLocalPed->IsInVehicle()) {
        VEHICLE_TYPE * remoteVehicle = reinterpret_cast<VEHICLE_TYPE *>(pLocalPed->m_pPed->pVehicle);

        CColStore::AddCollisionNeededAtPosn(&playerPos);
        CIplStore::AddIplsNeededAtPosn(&playerPos);

        const auto& removeVehiclePos = ((ENTITY_TYPE *)remoteVehicle)->m_placement.m_vPosn;
