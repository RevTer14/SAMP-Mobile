...
struct sigaction act_old, act1_old, act2_old, act3_old;
...
void handler(int signum, siginfo_t *info, void* contextPtr)
{
    ...
    if (act_old.sa_sigaction) act_old.sa_sigaction(signum, info, contextPtr);
...
{
    ucontext* context = (ucontext_t*)contextPtr;
    if (act1_old.sa_sigaction) act1_old.sa_sigaction(signum, info, contextPtr);
...
{
    ucontext* context = (ucontext_t*)contextPtr;
    if (act2_old.sa_sigaction) act2_old.sa_sigaction(signum, info, contextPtr);
...
{
    ucontext* context = (ucontext_t*)contextPtr;
    if (act3_old.sa_sigaction) act3_old.sa_sigaction(signum, info, contextPtr);
...
void DoInitStuff()
{
    ...
    if (!bNetworkInited && !bDebug)
    {
        ...
        if (pSettings)
        {
            ...
            bNetworkInited = true;
            FLog("Connected to %s:%d", pSettings->Get().szHost, pSettings->Get().iPort);
        }
        else
        {
...
extern "C" {
    ...
    JNIEXPORT void JNICALL Java_com_samp_mobile_game_ui_dialog_DialogManager_sendDialogResponse(JNIEnv* pEnv, jobject thiz, jint i3, jint i, jint i2, jbyteArray str)
    {
        ...
        if(pNetGame) pNetGame->SendDialogResponse(i, i3, i2, (char*)szStr.c_str());
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    // ❌ أزلت Firebase — إذا أردت إعادتها، فعّلها في build.gradle أولاً
...
void FLog(const char* fmt, ...)
{
    ...
    LOGI("%s", buffer);
    // ❌ أزلت: firebase::crashlytics::Log(buffer);
