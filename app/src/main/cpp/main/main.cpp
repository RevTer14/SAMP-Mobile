...
void LoadCleoLibrary(void* reserved)
{
	LOGI("Loading cleo library..");

	void *cleo_lib = dlopen("/data/data/com.samp.mobile/lib/libcleo.so", RTLD_LAZY);
	if (!cleo_lib)
	{
		LOGI("Failed to load cleo library. (error: %s)", dlerror());
		return;
	}

	((jint(*)(JavaVM *, void *))(dlsym(cleo_lib, "JNI_OnLoad")))(javaVM, reserved);
	LOGI("Cleo library loaded!");
}
...
void LoadBassLibrary() {
	void *v0 = dlopen("/data/data/com.samp.mobile/lib/libbass.so", 1);
